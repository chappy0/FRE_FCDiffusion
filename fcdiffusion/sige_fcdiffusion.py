...
from ldm.modules.diffusionmodules.openaimodel import  TimestepEmbedSequential,Downsample, AttentionBlock
from ldm.modules.diffusionmodules.sige_openaimodel import SIGEResBlock,SIGEDownsample
...
class ControlledUnetModel(SIGEUNetModel):
        ...
        with torch.no_grad():
            ...
            for module in self.input_blocks:
                if isinstance(module, TimestepEmbedSequential):
                    h = module(h, emb=emb, context=context)  # 确保传递emb和context
                else:
                    h = module(h)
                hs.append(h)
            h = self.middle_block(h, emb=emb, context=context)  # 确保传递emb和context
...
class FreqControlNet(nn.Module):
    def __init__(
        ...
        print(f"fcdnet params:{image_size,in_channels,model_channels,num_res_blocks,attention_resolutions,dropout,channel_mult,conv_resample,dims,use_checkpoint,use_fp16,num_heads,num_head_channels,num_heads_upsample,use_scale_shift_norm,SIGEResBlock_updown,use_new_attention_order,use_spatial_transformer,transformer_depth,context_dim,n_embed,legacy,disable_self_attentions,num_attention_blocks,disable_middle_self_attn,use_linear_in_transformer}")
        super().__init__()
                ...
                if ds in attention_resolutions:
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            ...
                            ) if not use_spatial_transformer else SIGESpatialTransformer(
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else SIGESpatialTransformer(  # always uses a self-attn
                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
