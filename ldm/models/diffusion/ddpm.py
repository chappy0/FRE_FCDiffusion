...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        # print(f"self.l_simple_weight,loss_simple,loss_vlb,self.original_elbo_weight,loss:{self.l_simple_weight,loss_simple,loss_vlb,self.original_elbo_weight,loss}")
    ...
    def get_input(self, batch, k):
        ...
        x = batch [k]
        # print(f"x:{x}")
        # print(f"ddpm get_input k:{k}")
        # print(f"ddpm get_input x,k:{x.shape}")
        if len(x.shape) == 3:
            x = x[..., None]
    ...
    def shared_step(self, batch):
        ...
        #function_name = sys._getframe().f_code.co_name
        #print(f"Executing {function_name} in class {class_name}")
        # print(f"ddpm shared_step x:{x.shape}")
    ...
    def training_step(self, batch, batch_idx):
        ...
        for k in self.ucg_training:
            # print(f"ucg_training:{ucg_training}")
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c):
        ...
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                # print(f"get_learned_conditioning c:{type(c)}")
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # print(f"ddpm return_first_stage_outputs:{return_first_stage_outputs,force_c_encode,return_x,return_original_cond}")
        # print(f"ddpm return_first_stage_outputs:{cond_key,self.cond_stage_key,self.first_stage_key}")
        #class_name = self.__class__.__name__
        #function_name = sys._getframe().f_code.co_name
        #print(f"Executing {function_name} in class {class_name}")
        ...
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        # print(f"LatentDiffusion get_intput x:{x.shape}")
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox', "txt"]:
                    # print(f"super get_input cond_key:{cond_key}")
                    xc = batch[cond_key]
                elif cond_key in ['class_label', 'cls']:
                    xc = batch
                else:
                    # print(f"super get_input xc:{cond_key}")
    ...
    def shared_step(self, batch, **kwargs):
        ...
        # print(f"lat shared_step x:{x.shape}")
        loss = self(x, c)
        # print(f"lat loss:{loss}")
        
        return loss

    def forward(self, x, c, *args, **kwargs):
        # print(f"self.shorten_cond_schedule,self.cond_stage_trainable:{self.shorten_cond_schedule,self.cond_stage_trainable}")
        #class_name = self.__class__.__name__
        #function_name = sys._getframe().f_code.co_name
        #print(f"Executing {function_name} in class {class_name}")
        ...
        if self.model.conditioning_key is not None:
            ...
            # print(f"lat self.model.conditioning_key:{self.model.conditioning_key}")
            if self.cond_stage_trainable:
                # print("run self.cond_stage_trainable")
                c = self.get_learned_conditioning(c)
            if self.shorten_cond_schedule:  # TODO: drop this option
                # print("run self.shorten_cond_schedule ")
    ...
    def p_losses(self, x_start, cond, t, noise=None,return_eps=False):
        ...
        # print(f"lat self.l_simple_weight,loss_simple,loss_vlb,self.original_elbo_weight,loss:{self.l_simple_weight,loss_simple,loss_vlb,self.original_elbo_weight,loss}")
        return loss, loss_dict
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:
            ...
            print (f"sample_log shape:{shape}")
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        ...
        if self.conditioning_key is None:
            ...
            print(f"x.shape,c_concat:{x.shape,c_concat}")
