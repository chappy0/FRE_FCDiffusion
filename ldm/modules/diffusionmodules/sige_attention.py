...
from ldm.modules.diffusionmodules.sige.nn import Gather, Scatter, SIGEConv2d, SIGEModule
...
class SIGECrossAttention(SIGEModule):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        # print(f"before context:{context}")
        context = default(context, x)
        # print(f"after context:{x.shape,context.shape}")
        if self.mode == "full":
            k = self.to_k(context)
            v = self.to_v(context)
            self.cached_k = k
