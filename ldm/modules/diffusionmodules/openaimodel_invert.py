...
class Upsample_origin(nn.Module):
    """
...
class Upsample(nn.Module):
    def __init__(self, channels, use_conv=True, dims=2, out_channels=None, mode='bilinear'):
        super().__init__()
        self.dims = dims
        self.out_channels = out_channels or channels
        
        # 根据维度选择卷积类型
        conv_class = nn.Conv2d if dims == 2 else nn.Conv3d
        
        self.up = nn.Sequential(
            nn.Upsample(scale_factor=2, mode=mode, align_corners=False),
            conv_class(channels, self.out_channels, 3, padding=1) if use_conv else nn.Identity(),
            nn.GroupNorm(8, self.out_channels) if use_conv else nn.Identity(),
            nn.GELU() if use_conv else nn.Identity()
        )
...
class Downsample_origin(nn.Module):
    """
...
class Downsample(nn.Module):
    """
    A downsampling layer with an optional convolution and hybrid pooling.
    :param channels: channels in the inputs and outputs.
    :param use_conv: a bool determining if a convolution is applied.
    :param dims: determines if the signal is 1D, 2D, or 3D. If 3D, then
                 downsampling occurs in the inner-two dimensions.
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1):
        ...
        if use_conv:
            ...
            self.mix_conv = nn.Conv2d(self.channels * 2, self.out_channels, kernel_size=1)
...
class ResBlock_origin(TimestepBlock):
    ...
    def __init__(
        ...
        if up:
            self.h_upd = Upsample_origin(channels, False, dims)
            self.x_upd = Upsample_origin(channels, False, dims)
        elif down:
            self.h_upd = Downsample_origin(channels, False, dims)
            self.x_upd = Downsample_origin(channels, False, dims)
        else:
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio=4, use_se=True):  # 降低扩展率至4
        super().__init__()
        ...
        self.conv = nn.Sequential(
            ...
            # 轻量级通道注意力
            SqueezeExcitation(hidden_dim, reduction=8) if use_se else nn.Identity(),
    ...
    def forward(self, x):
        if self.use_res_connect:
            return x + self.conv(x)  # 残差连接增强梯度流动
        else:
            return self.conv(x)

class HybridInvertedResidual(InvertedResidual):
    def __init__(self, inp, oup, stride, expand_ratio=4, emb_channels=None):  # 统一扩展率
        super().__init__(inp, oup, stride, expand_ratio)
        
        # 精简条件投影
        self.cond_proj = nn.Sequential(
            nn.SiLU(),
            linear(emb_channels, oup)  # 单投影代替scale+shift
        )

    def forward(self, x, emb):
        x = super().forward(x)
        cond = self.cond_proj(emb)
        return x * (1 + cond[:, :, None, None])  # 仅用乘法调制
...
class ResBlock(TimestepBlock):
    def __init__(
        ...
        # 使用 InvertedResidual 替代原有的特征提取部分
        # self.inverted_residual = InvertedResidual(channels, self.out_channels, 1, expand_ratio, use_se)
        # 增加 expand_ratio 来提升模型容量
        # self.inverted_residual = HybridInvertedResidual(channels, self.out_channels, 1, expand_ratio=8,emb_channels=emb_channels)
        self.inverted_residual = HybridInvertedResidual(
            channels, self.out_channels, 
            stride=1, 
            expand_ratio=4,  # 统一使用适度扩展
        ...
        self.emb_layers = nn.Sequential(
            nn.SiLU(),
            linear(emb_channels, self.out_channels)  # 维度对齐
        )
    ...
    def _forward(self, x, emb):
        ...
        if self.updown:
            x = self.x_upd(x)
        h = self.inverted_residual(x,emb)  # 使用 InvertedResidual 进行特征提取
            ...
            for i in range(self.num_res_blocks[level] + 1):
                ...
                if level and i == self.num_res_blocks[level]:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample(channels=ch,use_conv=conv_resample,dims=dims,out_channels=out_ch,mode='bilinear')
                    )
                    ds //= 2
...
class UNetModel_origin(nn.Module):
    ...
    :param model_channels: base channel count for the model.
    :param out_channels: channels in the output Tensor.
    :param num_res_blocks: number of residual blocks per Downsample_origin.
    :param attention_resolutions: a collection of Downsample_origin rates at which
        attention will take place. May be a set, list, or tuple.
        For example, if this contains 4, then at 4x downsampling, attention
        will be used.
    :param dropout: the dropout probability.
    ...
    :param num_heads_channels: if specified, ignore num_heads and instead use
                               a fixed channel width per attention head.
    :param num_heads_Upsample_origin: works with num_heads to set a different number
                               of heads for upsampling. Deprecated.
    :param use_scale_shift_norm: use a FiLM-like conditioning mechanism.
    :param ResBlock_origin_updown: use residual blocks for up/downsampling.
    :param use_new_attention_order: use a different attention pattern for potentially
                                    increased efficiency.
    """
    ...
    def __init__(
        ...
        if num_heads_Upsample_origin == -1:
            num_heads_Upsample_origin = num_heads
        ...
        self.num_heads = num_heads  # -1
        self.num_head_channels = num_head_channels  # 64
        self.num_heads_Upsample_origin = num_heads_Upsample_origin  # -1
        self.predict_codebook_ids = n_embed is not None  # False
            ...
            for i in range(self.num_res_blocks[level] + 1):
                    ...
                    if not exists(num_attention_blocks) or i < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ch,
                                use_checkpoint=use_checkpoint,
                                num_heads=num_heads_Upsample_origin,
                                num_head_channels=dim_head,
                                use_new_attention_order=use_new_attention_order,
                            ) if not use_spatial_transformer else SpatialTransformer(
                                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                ...
                if level and i == self.num_res_blocks[level]:
                    ...
                    layers.append(
                        ...
                        if ResBlock_origin_updown
                        else Upsample_origin(ch, conv_resample, dims=dims, out_channels=out_ch)
                    )
                    ds //= 2
                self.output_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
